# Deploy SolSignal on Oracle Cloud Always-Free ($0, 24/7)
...
```bash
sudo mkdir -p /opt/solsignal && sudo chown $USER /opt/solsignal
git clone <YOUR_REPO_URL> /opt/solsignal
cd /opt/solsignal
cp .env.example .env
```
...
- `SOLSIGNAL_DOMAIN=trade.yourdomain.com` and `CADDY_ACME_EMAIL=you@example.com`
...
Let's Encrypt cert for `SOLSIGNAL_DOMAIN` within a minute.
...
```bash
sudo cp deploy/solsignal.service /etc/systemd/system/solsignal.service
sudo systemctl daemon-reload
sudo systemctl enable --now solsignal
```
...
docker logs solsignal-healthwatch     # bots ok, dry_run tripwire armed
curl -s https://trade.yourdomain.com -o /dev/null -w "%{http_code}\n"  # 200
```

## Ongoing

- **Update**: `cd /opt/solsignal && git pull && ./scripts/deploy.sh`
